...
public class withdraw_button implements ActionListener {
  ...
  public withdraw_button(String id, String username) {
    ...
    this.depButton = new JButton(" Withdraw ");
  ...
  public void actionPerformed(ActionEvent e) {
    if (e.getSource() == this.depButton) {
      ...
      if (val) {
        boolean boll1 = JdbcConnector.database_deduct_balance(this.id, dep);
        if (boll1) {
          JdbcConnector.database_enter_record(this.id, "0", dep, "Withdraw");
          String amountrem = JdbcConnector.getBalance(this.id);
          JOptionPane.showMessageDialog(null, "Withdrawn successfully! Your balance is " + amountrem + ".");
